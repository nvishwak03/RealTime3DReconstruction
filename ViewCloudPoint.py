...
while True:
    ...
    if len(points) > 0:
        ...
        # Compute mesh using Poisson surface reconstruction
        try:
            new_mesh, densities = o3d.geometry.TriangleMesh.create_from_point_cloud_poisson(pcd_down, depth=5)
            new_mesh.compute_vertex_normals()
        except Exception as e:
            print(f"Mesh reconstruction failed: {e}")
            new_mesh = None
        
        # Manage mesh in visualizer
        ...
        # Render the point cloud and mesh
        vis.poll_events()
        vis.update_renderer()
    else:
        print("Warning: No valid points for point cloud or mesh.")
        if current_mesh is not None:
            vis.remove_geometry(current_mesh)
            current_mesh = None
